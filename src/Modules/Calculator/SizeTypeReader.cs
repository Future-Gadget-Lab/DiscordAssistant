...
namespace Assistant.Modules.Calculator
{
    ...
    {
        private static readonly string FailMessage = "Invalid size provided, please provide a size in format WIDTHxHEIGHT or a single number for a square graph.";
        ...
        public override Task<TypeReaderResult> ReadAsync(ICommandContext context, string input, IServiceProvider services)
        {
            ...
            if (!int.TryParse(chars.Slice(0, xPos), out int width) || !int.TryParse(chars.Slice(xPos + 1), out int height))
                return Task.FromResult(TypeReaderResult.FromError(CommandError.ParseFailed, FailMessage));
